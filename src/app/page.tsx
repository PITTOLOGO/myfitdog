...
export default function Home() {
  ...
  // redirect a /login quando authReady && non loggato
  useEffect(() => {
    if (!authReady) return;
    if (uid) return;
    ...
    const t = setTimeout(() => {
      // fallback “hard” se la route si incarta
  ...
  // carica cane attivo + calcolo target kcal
  useEffect(() => {
    async function loadDogAndCalc() {
      ...
      const season =
        (d.seasonFactor ?? "auto") === "auto"
          ? deriveSeasonFromMonth()
          : (d.seasonFactor ?? "mild");
  ...
  // non loggato -> redirect già in corso
  if (!uid) return null;
      ...
      <div className="grid gap-3 mt-4">
        ...
        {coachInsight ? (
          <PremiumCard
            ...
            subtitle={
              activeDogName
                ? "Consigli basati sui tuoi ultimi giorni"
